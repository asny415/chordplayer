...
struct PlayingPatternEditorView: View {
    ...
    private func save() {
        ...
        for col in 0..<cols {
            ...
            if modeIsStrum {
                guard let dir = (strumDirections.indices.contains(col) ? strumDirections[col] : nil) else { continue }
                let fixedOrder: [Int] = (dir == "down") ? [1,2,3,4,5,6] : [6,5,4,3,2,1]
                ...
                for string in 0..<stringCount {
                    if grid[string][col] {
                        if let m = markers[string][col] {
                            ...
                            } else if m.hasPrefix("FIXED:"), let num = Int(m.split(separator: ":")[1]) {
                                notesForCol.append(.chordString(num))
                            } else if m.hasPrefix("FRET:"), let fret = Int(m.split(separator: ":")[1]) {
